...
function processRawData(rows){
  ...
  for(i=1;i<rows.length;i++){
    ...
    if(firstVal.indexOf(".")>=0&&firstVal.length>5)continue;
    if(!firstVal&&!String(r[1]||"").trim())continue;
  ...
  gel("pshin").textContent=CAT.length+" productos disponibles";console.log("DAILY:"+DAILY.length+" MONTHLY:"+MONTHLY.length+" CAT:"+CAT.length);if(DAILY.length>0)console.log("First daily:"+JSON.stringify(DAILY[0]));
  return true;
}
