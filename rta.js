...
function pF(v){if(v===undefined||v===null||v==="")return 0;var s=String(v).trim();if(!s||s==="nan")return 0;if(s.indexOf(",")>=0){s=s.replace(/\./g,"").replace(",",".");}try{return parseFloat(s)||0;}catch(e){return 0;}}
...
function processRawData(rows){
  ...
  for(i=0;i<headers.length;i++){
    ...
    COL[hk]=i;
    var hkn=hk.replace(/[^a-zA-Z0-9 ]/g,"").toLowerCase();
    COL["_n_"+hkn]=i;
  }
  function gc(names,fb){
    var ns=names.split("|"),j,k,nk;
    for(j=0;j<ns.length;j++){
      k=ns[j];
      if(COL[k]!==undefined)return COL[k];
      nk=k.replace(/[^a-zA-Z0-9 ]/g,"").toLowerCase();
      if(COL["_n_"+nk]!==undefined)return COL["_n_"+nk];
    }
    return fb;
  }
  var n=headers.length;
  var hhI=gc("hh Total",53),m2I=gc("M2 Total",41),mlI=gc("ml Total",54),
      ppI=gc("Total Piezas Perforadas",57),cepI=gc("Cantidad CEP",37),
      unI=gc("Unidades Total",39),cdI=gc("CODIGO",8),ofI=gc("OF Asignada",33),
      dtI=gc("FECHA ENTREGA",2),
      yrI=gc("Ano|ANO|Year",3),
      moI=gc("Mes|MES|Month",4),
      wkI=gc("semana|Semana|SEMANA|Week",5),
      descI=gc("DESCRIPCION|DESCRIPCION",9),
      famI=gc("FAMILIA|familia",68),
      catI=gc("Categoria Nivel 0|categoria nivel 0",67);

  var dayMap={},prodMap={};
  for(i=1;i<rows.length;i++){
    r=rows[i];
    if(!r||r.length<3){continue;}
    if(!String(r[yrI]||r[dtI]||"").trim()){continue;}
    ...
    if(!yr||!mo2||isNaN(yr)||isNaN(mo2)){continue;}
    ...
    if(!cd&&un===0&&hh===0&&m2v===0){continue;}
  ...
  gel("pshin").textContent=CAT.length+" productos disponibles";if(DAILY.length>0)
  return true;
}
